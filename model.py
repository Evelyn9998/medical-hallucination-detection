...
class MedicalHallucinationDetector:
    ...
    def train_model(self, train_texts, train_labels, val_texts, val_labels,
        ...
        print(f"Model: Bio_ClinicalBERT (medical domain optimized)")
    ...
    def _print_memory_efficient_summary(self, trainer):
        """Print a summary of training optimizations"""
        print("\n" + "="*60)
        print("MEDICAL HALLUCINATION DETECTION - TRAINING SUMMARY")
        print("="*60)
        print("✓ Model Configuration:")
        print("  - Using Bio_ClinicalBERT (medical domain pre-trained model)")
        print("  - Full sequence length (512 tokens) for complete medical text")
        print("  - Optimized batch size (8) for balanced performance")
        print("  - Medical domain vocabulary for accurate understanding")
        print()
        ...
        print("  - Gradient accumulation (2 steps) for effective batch size")
        print("  - Gradient clipping (max_norm=1.0) for stability")
        print("  - Mixed precision training (FP16) when CUDA available")
        print()
        print("✓ Medical Domain Optimizations:")
        print("  - Dropout optimization (0.2 hidden, 0.2 attention, 0.3 classifier)")
